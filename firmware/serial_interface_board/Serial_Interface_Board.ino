...
//
//                      8. Add keyboard key which aborts current printout and flushes print & input buffers.
//
//======================================================================================================================
...
//                     d.  Processing all print characters transfered from the ISRs.
...
//                                     inject "fake" keyboard key pressses to the Wheelwriter logic board for printing.
//
...
//                    Therefore, classic semaphores and mutexes cannot be used to guard critial code segments in the
...
//             terminal it can cause loss of input data from the actual keyboard.  This firmware trys hard to keep from
...
//                          +-------------------+         +-------------------+         +-------------------+
//
//                    Creating a period grapic character involves combining explicit and implicit carriage movements
...
void Update_ASCII_settings () {
  ...
  if ((oserial != serial) || (oparity != parity) || (obaud != baud) || (odps != dps)) {
    flow_in_on = FALSE;
    flow_out_on = FALSE;
    if (oserial == SERIAL_USB) {
      Serial.end ();
    } else /* oserial == SERIAL_RS232 */ {
      Serial1.end ();
    }
    if (serial == SERIAL_USB) {
      Serial.begin (115200);
...
void Initialize_configuration_settings (boolean reset) {
  ...
  if (serial == SERIAL_USB) hwflow = HWFLOW_NONE;
}
...
byte Read_baud_setting (const char str[], byte value) {
  ...
  while (TRUE) {
    ...
    if (chr == '1') {
      ...
      val = BAUD_57600;
      Print_characters ("57600\r");
      break;

    } else if (chr == '7') {
      val = BAUD_76800;
      Print_characters ("76800\r");
      break;
      ...
      val = value;
      Print_unsigned_long (baud_rates[val], 0);
      Print_character ('\r');
      break;
    }
  }
...
// Send no serial communication port.
inline void Serial_send_now () {
  if (serial == SERIAL_USB) {
    Serial.send_now ();
  } else /* serial == SERIAL_RS232 */ {
    // UART serial does not support send_now.
  }
}
